...
export class Client extends Base {
    ...
    async createSpace(name, options) {
        ...
        if (options.skipGatewayAuthorization !== true) {
            if (!options.authorizeGatewayServices ||
                options.authorizeGatewayServices.length === 0) {
                throw new Error('failed to authorize Gateway Services: missing <authorizeGatewayServices> option');
            }
...
export const authorizeContentServe = async (client, space, connection, options = {}) => {
    ...
    try {
        ...
        if (verificationResult.out.error) {
            throw new Error(`failed to publish delegation for audience ${options.audience}: ${verificationResult.out.error.message}`, {
