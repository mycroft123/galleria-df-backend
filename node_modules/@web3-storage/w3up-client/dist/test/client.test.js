...
import { Agent, AgentData, claimAccess, requestAccess, } from '@web3-storage/access/agent';
...
import { alice, confirmConfirmationUrl, gateway, } from '../../upload-api/test/helpers/utils.js';
...
export const testClient = {
    uploadFile: Test.withContext({
        'should upload a file to the service': async (assert, { connection, provisionsStorage, uploadTable, allocationsStorage }) => {
            ...
            assert.deepEqual(await allocationsStorage.exists(space.did(), expectedCar.cid.multihash), {
                ok: true,
            });
    ...
    uploadCar: Test.withContext({
        'uploads a CAR file to the service': async (assert, { connection, provisionsStorage, uploadTable, allocationsStorage }) => {
            ...
            assert.deepEqual(await allocationsStorage.exists(space.did(), carCID.multihash), {
                ok: true,
            });
        },
    }),
    ...
    spaces: Test.withContext({
        ...
        'should fail to create a space due to delegate access error': async (assert, { client, mail, connect, grantAccess }) => {
            ...
            // Step 3: Attempt to create a space with the account
            await assert.rejects(client.createSpace('delegate-fail-space-test', {
                account: aliceAccount,
            }), {
    ...
    authorizeGateway: Test.withContext({
        ...
        'should throw when the content serve authorization fails due to missing service configuration': async (assert, { mail, grantAccess, connection }) => {
            // Step 1: Create a client for Alice and login
            const aliceClient = new Client(await AgentData.create({
                principal: alice,
            }), {
            ...
            try {
                ...
                assert.match(
                // @ts-expect-error
                error.message, /missing <authorizeGatewayServices> option/, 'should throw when creating the space');
            }
        },
